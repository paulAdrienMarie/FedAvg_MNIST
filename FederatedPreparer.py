...
class FederatedPreparer:
    ...
    def generate(self):
        ...
        # should be on top of the module
        ...
        # dataset = [self.image_message(img)["url"] for img in images]
        dataset = []
    ...
    def prepare_jsons_for_federated_learning(self, images, labels):
        ...
        # should use a root url (arg of the class?)
        output_dir = "./static/dataset/"
        os.makedirs(output_dir, exist_ok=True)

        # Clear the output directory if it is not empty

        # if we ignore failures to delete a file, just do shutil.rmtree(output_dir, ignore_errors=True) and get rid of this whole loop
        for filename in os.listdir(output_dir):
            ...
            try:
                ...
                # we ignore failure to delete a file?
        ...
        for user_id in range(self.nb_users):
            ...
            for i, label in enumerate(pictures_labels):
                ds.append({"label": label, "base64": pictures_base64[i]})
    ...
    def prepare_training_artifacts(self):
        ...
        # use a root dir + os.path.join
        # use a better name for that instance. artifacts?
        obj = Artifacts("./onnx/inference.onnx")

        # that is cryptic. use an explicit method name like "generate" ?
        # artifacts.generate()
        obj()
