...
# use a better name Test is too generic
class Test:
    """
    ...
    def __init__(self):
        ...
        self.metrics = {"accuracies": [], "losses": []}

    def load_test_images(self):
        """Loads test set of images from the MNIST dataset"""
        # should be a CONSTANT on the top of the file
        batch_size = 64
        train_kwargs = {"batch_size": batch_size}

        # what are these values? use explicit constants
        ...
        # you don't need the variable, return directly
    ...
    def get_predicted_labels(self, logits):
        ...
        return np.argmax(
            logits, axis=-1
        )  # Assuming logits is a 2D array (batch_size, num_classes)
    ...
    def test_artifacts(self, test_loader):
        ...
        # I think you want a root dir as a param of the class and then use it
        # us os.path.join to build the paths
        ...
        module = Module(
            ...
            # can we use cuda or mps if available on the hardware ?
            device="cpu",
        )
        ...
        # if you don't use the index, drop enumerate and remove the _
        for _, (data, target) in enumerate(test_loader):
            ...
            metric.add_batch(
                references=target, predictions=self.get_predicted_labels(logits)
            )
